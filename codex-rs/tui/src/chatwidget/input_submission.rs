        ...
        } = &mut op
        {
            ...
            if self.config.ultracode && !self.config.disable_workflows {
                context.insert("ultracode_session".to_string(),codex_app_server_protocol::AdditionalContextEntry {
                    ...
                    value:"The user enabled Ultracode for this session. Use the built-in tools.mcp__codex_tui__workflow tool and its authoring instructions to carry out the request; no plugin or skill lookup is needed. The parent uses xhigh reasoning; explicit worker model and effort choices remain independent.".to_string(),
                });
            }
            if let Some(enabled) = workflow_keyword {
                context.insert("ultracode_keyword".to_string(), codex_app_server_protocol::AdditionalContextEntry {
                    kind: codex_app_server_protocol::AdditionalContextKind::Application,
                    value: if enabled {
                        "The user enabled Ultracode for this message through the interactive workflow keyword. Use the built-in tools.mcp__codex_tui__workflow tool and its authoring instructions to carry out the request; no plugin or skill lookup is needed. This choice does not change session reasoning effort."
                    } else {
