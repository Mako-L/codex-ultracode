...
pub fn workflow_backend_state_dir(codex_home: &Path, codex_bin: &Path) -> Result<PathBuf> {
    ...
    hasher.update(codex_bin.as_os_str().as_encoded_bytes());
    hasher.update(&[0]);
...
pub async fn ensure_workflow_backend(codex_home: &Path, codex_bin: &Path) -> Result<PathBuf> {
    ...
    let daemon = Daemon {
        ...
        pid_file: state_dir.join(DAEMON_PID_FILE_NAME),
        update_pid_file: state_dir.join(DAEMON_UPDATE_PID_FILE_NAME),
