...
fn update_loop_uses_hidden_app_server_subcommand() {
    let backend = PidBackend::new_update_loop("codex".into(), "updater.pid".into(), None);

    assert_eq!(
        backend.command_args(),
        vec!["app-server".to_owned(), "daemon".to_owned(), "pid-update-loop".to_owned()]
    );
}
...
fn app_server_remote_control_uses_runtime_flag() {
    ...
    assert_eq!(
        backend.command_args(),
        vec!["app-server".to_owned(), "--remote-control".to_owned(), "--listen".to_owned(), "unix://".to_owned()]
    );
}
...
fn app_server_disabled_remote_control_uses_compatible_args_and_runtime_env() {
    ...
    assert_eq!(
        backend.command_args(),
        vec!["app-server".to_owned(), "--listen".to_owned(), "unix://".to_owned()]
    );
    assert_eq!(
        backend.command_env(),
